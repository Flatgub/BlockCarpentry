...
public class ChestFrameBlock extends FrameBlock implements IWaterLoggable {
    ...
    public ChestFrameBlock(Properties properties) {
        super(properties);
        this.registerDefaultState(this.getDefaultState().with(CONTAINS_BLOCK, false).with(LIGHT_LEVEL, 0).with(HORIZONTAL_FACING, Direction.NORTH).with(WATERLOGGED, false));
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World world, BlockPos pos, PlayerEntity player,
        ...
        if (!world.isRemote) {
            ...
            if (item.getItem() instanceof BlockItem) {
                ...
                if (tileEntity instanceof ChestFrameBlockEntity && !item.isEmpty() && BlockSavingHelper.isValidBlock(heldBlock) && !state.get(CONTAINS_BLOCK)) {
            ...
            if (tileEntity instanceof ChestFrameBlockEntity && state.get(CONTAINS_BLOCK)) {
                if (!(Objects.requireNonNull(item.getItem().getRegistryName()).getNamespace().equals(BlockCarpentryMain.MOD_ID))) {
